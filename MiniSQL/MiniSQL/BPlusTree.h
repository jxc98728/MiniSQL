...
template<typename T>
BPlusTreeNode<T>::BPlusTreeNode(int degree, bool isLeaf) : degree(degree), isLeaf(isLeaf), cnt(0), parent(nullptr),
sibling(nullptr) {
...
template<typename T>
bool BPlusTreeNode<T>::search(const T &key, int &index) const {
	if (cnt == 0) {
		index = 0;
		return false;
	}
...
template<typename T>
bool BPlusTreeNode<T>::binarySearch(const T &key, int &index) const {
...
template<typename T>
BPlusTreeNode<T> *BPlusTreeNode<T>::split(T &key) {
	BPlusTreeNode<T> *newNode = new BPlusTreeNode<T>(degree, isLeaf);
...
template<typename T>
int BPlusTreeNode<T>::add(const T &key) {
	int index;
...
template<typename T>
int BPlusTreeNode<T>::add(const T &key, int offset) {
	int index;
	bool keyExists = search(key, index);
	if (keyExists) {
		cerr << "Key is not unique!" << endl;
...
template<typename T>
void BPlusTreeNode<T>::removeAt(int index) {
...
class BPlusTree {
public:
	typedef BPlusTreeNode<T> *TreeNode;

	BPlusTree(int sizeofKey, int degree);
...
template<typename T>
BPlusTree<T>::BPlusTree(int sizeofKey, int degree) : sizeofKey(sizeofKey),
...
template<typename T>
void BPlusTree<T>::initBPlusTree() {
...
template<typename T>
bool BPlusTree<T>::findKeyFromNode(TreeNode node, const T &key, NodeSearchParse<T> &res) {
	int index;
...
template<typename T>
NodeSearchParse<T> BPlusTree<T>::findNode(const T &key) {
	NodeSearchParse<T> res;
	if (!root) { return res; }
...
template<typename T>
bool BPlusTree<T>::insert(const T &key, int offset) {
	NodeSearchParse<T> res;
	if (!root) { initBPlusTree(); }
...
template<typename T>
void BPlusTree<T>::cascadeInsert(BPlusTree::TreeNode node) {
	T key;
	TreeNode sibling = node->split(key);
	nodeCount++;

	if (node->isRoot()) {
		TreeNode root = new BPlusTreeNode<T>(degree, false);
		level++;
		nodeCount++;
		this->root = root;
		node->parent = root;
...
template<typename T>
bool BPlusTree<T>::remove(const T &key) {
	NodeSearchParse<T> res;
	if (!root) {
		cerr << "Dequeuing empty BPlusTree!" << endl;
		return false;
	}
	if (!findKeyFromNode(root, key, res)) {
...
template<typename T>
bool BPlusTree<T>::cascadeDelete(BPlusTree::TreeNode node) {
...
template<typename T>
bool BPlusTree<T>::deleteBranchLL(BPlusTree::TreeNode node, BPlusTree::TreeNode parent, BPlusTree::TreeNode sibling, int index) {
...
template<typename T>
bool BPlusTree<T>::deleteBranchLR(BPlusTree::TreeNode node, BPlusTree::TreeNode parent, BPlusTree::TreeNode sibling, int index) {
...
template<typename T>
bool BPlusTree<T>::deleteBranchRL(BPlusTree::TreeNode node, BPlusTree::TreeNode parent, BPlusTree::TreeNode sibling, int index) {
...
template<typename T>
bool BPlusTree<T>::deleteBranchRR(BPlusTree::TreeNode node, BPlusTree::TreeNode parent, BPlusTree::TreeNode sibling, int index) {
...
template<typename T>
bool BPlusTree<T>::deleteLeafLL(BPlusTree::TreeNode node, BPlusTree::TreeNode parent, BPlusTree::TreeNode sibling, int index) {
	for (int i = node->cnt; i > 0; i--) {
		node->keys[i] = node->keys[i - 1];
...
template<typename T>
bool BPlusTree<T>::deleteLeafLR(BPlusTree::TreeNode node, BPlusTree::TreeNode parent, BPlusTree::TreeNode sibling, int index) {
	parent->removeAt(index);
	for (int i = 0; i < node->cnt; i++) {
...
template<typename T>
bool BPlusTree<T>::deleteLeafRL(BPlusTree::TreeNode node, BPlusTree::TreeNode parent, BPlusTree::TreeNode sibling, int index) {
...
template<typename T>
bool BPlusTree<T>::deleteLeafRR(BPlusTree::TreeNode node, BPlusTree::TreeNode parent, BPlusTree::TreeNode sibling, int index) {
	for (int i = 0; i < sibling->cnt; i++) {
		node->keys[node->cnt + i] = sibling->keys[i];
